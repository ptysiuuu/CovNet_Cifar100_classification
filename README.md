...
Dataset: CIFAR-100
🚀 Approach
Data Preparation: Loaded and preprocessed the CIFAR-100 dataset using PyTorch's torchvision.datasets.
Custom Model Development: Implemented a custom CNN with distinct backbone and classifier modules.
Pre-trained Model Comparison: Utilized the ResNet-18 architecture from torchvision.models as a pre-trained backbone for comparison.
Training and Evaluation: Trained both models, recorded performance metrics, and visualized the results.
Analysis: Compared the custom and pre-trained models based on accuracy, learning curves, and inference speed.
📊 Results
